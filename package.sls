  ...
  (cases <version-constraint> constraint
    ((version-between lower upper)
     (cond ((and lower upper)
            (and (package-version<=? lower version)
                 (package-version<=? version upper)))
           ((not lower)
            (package-version<=? version upper))
           ((not upper)
            (package-version<=? lower version))))
    ((version-not constraint)
...
(define (%form->constraint form lose)
  (match form
    (('<= version ___)
     (make-version-between #f (->package-version version lose)))
    (('>= version ___)
     (make-version-between (->package-version version lose) #f))
    (('not sub-form)
    ...
    (version
     (let ((version (->package-version/sloppy version lose)))
       (make-version-between version version)))))

(define (version-constraint->form constraint)
  (cases <version-constraint> constraint
    ((version-between start end)
